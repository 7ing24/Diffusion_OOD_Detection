...
class DiffusionModel(nn.Module):
    ...
    def sample(self, model, cond, n_action_samples, n_steps=100):
        """
        ...
        Args:
            ...
            n_action_samples (int): Number of samples per condition/state
            n_steps (int): Number of diffusion steps

        Returns:
            actions (torch.Tensor): Sampled actions, shape (B, n_action_samples, action_dim)
        """
        model.eval() 
        ...
        actions = torch.randn(B * n_action_samples, action_dim).to(self.device)
        ...
        cond = cond.unsqueeze(1).repeat(1, n_action_samples, 1).view(B * n_action_samples, cond_dim)

        with torch.no_grad():
            for i in range(n_steps):
                sigma = sigmas[i]
                sigma = sigma.expand(B * n_action_samples).unsqueeze(1)  # Expand sigma to match the batch size
        ...
        actions = actions.view(B, n_action_samples, action_dim)
        return actions
