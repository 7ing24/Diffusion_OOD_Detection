...
class ScoreNetwork(nn.Module):
    ...
    def get_embedding(self, x, cond, sigma, uncond=False):
        if len(sigma.shape) == 0:
            sigma = einops.rearrange(sigma, ' -> 1').unsqueeze(1)
        elif len(sigma.shape) == 1:
            sigma = sigma.unsqueeze(1)
        embed = self.embed(sigma).squeeze(1)
        if embed.shape[0] != x.shape[0]:
            embed = einops.repeat(embed, '1 d -> (1 b) d', b=x.shape[0])
        ...
        # 只取 MLP 最后一层前的输出，作为embedding
        for i, layer in enumerate(self.layers.layers[:-1]):
            x = layer(x)
        return x  # shape [B, D]