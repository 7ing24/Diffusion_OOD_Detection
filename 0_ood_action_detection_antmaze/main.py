...
def bc_warmup(agent, replay_buffer, warmup_steps, batch_size, device, lr):
    ...
    for step in range(warmup_steps):
        ...
        pred_action, _ = agent.actor(state, deterministic=False, need_log_prob=False)
        ...
        if step % 1000 == 0:
            print(f"[BC Warmup] step {step}/{warmup_steps}, loss = {loss.item():.4f}")
            wandb.log({"BC_warmup/loss": loss.item()}, step=step)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bc_warmup_steps", default=20000, type=int)
    parser.add_argument("--bc_warmup_lr", default=3e-4, type=float)
    ...
    print("========== 开始 BC warmup ==========")
    bc_warmup(agent, replay_buffer, args.bc_warmup_steps, args.batch_size, device, lr=args.bc_warmup_lr)
    print("========== BC warmup 完成 ==========")
